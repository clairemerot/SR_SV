...
VCF_FOLDER=05_manta/TMP_VCF
INPUT_VCF=05_manta/all/manta_SV.vcf.gz # we will try not to modify this one
OUTPUT_VCF=07_filtered_vcf/manta.vcf
...
grep -v ^\#\# $VCF_FOLDER/raw.vcf | wc -l  #143196


#filter out TRA & BND
bcftools filter -i'INFO/SVTYPE!="TRA" & INFO/SVTYPE!="BND"' -o $VCF_FOLDER/raw_sorted.noTRA.vcf -Ov $VCF_FOLDER/raw.vcf 
#grep -v ^\#\# $VCF_FOLDER/raw_sorted.noTRA.vcf | head
echo "total number of SVs restricted to INS, DEL, INV, DUP"
grep -v ^\#\# $VCF_FOLDER/raw_sorted.noTRA.vcf | wc -l #103682
#in reality we have not kept INV. To do so, we should first convert BND into inversions, but I don't fully trust them...

#filter vcf -i (include, -O vcf format -o
bcftools filter -i'INFO/SVLEN<=100000 && INFO/SVLEN>=-100000' -o $VCF_FOLDER/raw_sorted.noTRA_100k.vcf -Ov $VCF_FOLDER/raw_sorted.noTRA.vcf
echo "total number of SVs < 100kb"
grep -v ^\#\# $VCF_FOLDER/raw_sorted.noTRA_100k.vcf | wc -l #96337
...
grep -v ^\#\# $VCF_FOLDER/raw_sorted.noTRA_100k_50bp.vcf | wc -l #65286

#then we use the reference to get the sequence with a R scripts graciously provided by Marc-André Lemay
Rscript 01_scripts/Rscripts/add_explicit_seq_manta.r "$VCF_FOLDER/raw_sorted.noTRA_100k_50bp.vcf" "$VCF_FOLDER/raw_sorted.noTRA_100k_50bp_withSEQ.vcf" "02_info/genome.fasta"
...
grep -v ^\#\# $VCF_FOLDER/raw_sorted.noTRA_100k_50bp_withSEQ_Nfiltered.vcf | wc -l #65249
